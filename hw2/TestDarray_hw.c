...
int main(int argc, char* argv[])
  ...
  /* declare local variables */
   ...
   if (2 == argc) /* note that argc 2 means one argument given */
      ...
      if (NULL != DataFile)
        ...
        ********************************************************************/
        /* WRITE YOUR CODE HERE */
          CreateDArray(&TestDynamicArray,INITIAL_SIZE);//Initialize the dynamic array
 	  ReadData(DataFile, &TestDynamicArray);//Read all Data from text file
	...
	printf("First 6 entries \n");
	 for (lcv = 0; lcv < 6; lcv++) {
	   printf("Entry: %d %s \n", lcv, TestDynamicArray.Payload[lcv].String);
	}

	printf("Last 6 entries \n");
	   for (lcv = TestDynamicArray.EntriesUsed - 6; lcv < TestDynamicArray.EntriesUsed ; lcv++){
	printf("%d %s \n",lcv,TestDynamicArray.Payload[lcv].String);
   ...
   while (EOF != fscanf(InputFile, formatStr, String))
     {
      /* Insert code here to make sure the input data is not too long
       ...
       if (MAX_STR_LEN < strLen){
       printf(stderr, " The string is too long \n");
	   //Error code here 
      }
      else {
